...
// rank=4, degree=10, coefficients=88
// Expression: rank-4 separable Chebyshev over sqrt-warped inputs.
...
float sheen_lut_approx_mobile(float roughness, float cos_theta) {
    float r = 2.0 * sqrt(clamp(roughness, 0.0, 1.0)) - 1.0;
    float v = 2.0 * sqrt(clamp(cos_theta, 0.0, 1.0)) - 1.0;
    float y = 0.0;
